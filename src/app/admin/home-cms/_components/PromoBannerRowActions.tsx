...
type PromoBannerRowActionsProps = {
  banner: PromoBanner;
  onEdit: (banner: PromoBanner) => void;
};

export default function PromoBannerRowActions({
  banner,
  onEdit,
  ...
  const handleToggleActive = () => {
    ...
    startToggleTransition(async () => {
      try {
        await togglePromoBannerActiveAction(banner.id, next);
        setIsActive(next);
        router.refresh();
        toast.success(
          next ? 'Banner promo activado.' : 'Banner promo desactivado.'
        );
      } catch (err) {
        const message =
  ...
  const handleDelete = () => {
    ...
    startDeleteTransition(async () => {
      try {
        await deletePromoBannerAction(banner.id);
        setDeleteOpen(false);
        router.refresh();
        toast.success('Banner promo eliminado.');
      } catch (err) {
        const message =
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogTrigger
          render={
            <Button
              ...
              size="sm"
              disabled={isBusy}
              aria-label="Eliminar banner promo"
            />
          }
        >
          <Trash2 />
        ...
        <AlertDialogContent className="bg-card">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Se borrará de la portada. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
