...
export default function ProductCard({ product }: ProductProps) {
  ...
  return (
    <Card className="group relative h-full flex flex-col overflow-hidden bg-background border-border hover:-translate-y-1 hover:shadow-[0_0_30px_rgba(159,192,48,0.15)] transition-all duration-300">
      ...
      {/* Badge Agotado */}
      {isOutOfStock && (
        <Badge variant="destructive" className="absolute top-4 right-4 z-10 shadow-md pointer-events-none uppercase tracking-widest font-extrabold text-[10px]">
          Agotado
        </Badge>
      )}
      ...
      <CardHeader className="p-5 pb-0 flex-1">
        {product.categories?.name && (
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-1 block font-sans">
            {product.categories.name}
          </span>
        )}
        ...
        {specKeys.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-3">
            {specKeys.map(key => (
              <Badge key={key} variant="secondary" className="text-[10px] font-body bg-secondary/50 hover:bg-secondary/80 text-secondary-foreground truncate max-w-[120px]">
      ...
      <CardFooter className="p-5 pt-6 mt-auto flex items-center justify-between border-t border-border/30 bg-[#0a0a0a]/50">
        ...
        <Button 
          ...
          className={`
            rounded-full font-bold transition-all duration-300 flex items-center gap-2
